...
/**
 * 스팩 보유현황 — 실제 보유(shares>0) 종목의 별도 관리 영역.
 * 보유수·평가액·매수단가 대비 현재가·평가손익(사실 기록, 매매 추천 아님).
 * 관심 스팩 목록에서 보유는 제외되므로 수정·삭제도 여기서.
 */
import { useState } from 'react'
import { Layers } from 'lucide-react'
import { cn, formatLargeNumber } from '@/lib/utils'
import { Card, CardContent } from '@/components/ui/card'
import { SpacForm, EditBtn, DeleteBtn } from '@/components/ipo/entry-forms'
...
export function SpacHoldings({ data }: { data: IpoData }) {
  const [editingId, setEditingId] = useState<string | null>(null)
  const held = data.spacs.filter(s => s.shares && s.shares > 0)
    ...
    <Card>
      <CardContent className="pt-4 space-y-3">
        ...
        <div className="divide-y divide-border/60">
          {held.map(s => {
            if (editingId === s.id) {
              return <div key={s.id} className="py-2"><SpacForm data={data} initial={s} onDone={() => setEditingId(null)} /></div>
            }
            ...
            return (
              /* 모바일: 2줄 flex-wrap(종목·평가액 / 보유·단가경로·액션) — 12칸 grid는 sm+ */
              <div key={s.id} className="flex flex-wrap items-center gap-x-2 gap-y-1 py-2 text-sm sm:grid sm:grid-cols-12 sm:gap-2">
                ...
                <span data-priv className="ml-auto sm:ml-0 text-right tabular-nums whitespace-nowrap sm:col-span-3">
                ...
                <span data-priv className="text-xs tabular-nums text-muted-foreground whitespace-nowrap sm:col-span-2 sm:text-right sm:text-sm">{s.shares!.toLocaleString()}주</span>
                <span className="text-xs tabular-nums text-muted-foreground whitespace-nowrap sm:col-span-2 sm:text-right">
                  {s.avgCost ? `${s.avgCost.toLocaleString()} → ` : ''}<span className="text-foreground">{s.price.toLocaleString()}</span>
                </span>
                <span className="ml-auto sm:ml-0 flex justify-end items-center gap-1.5 sm:col-span-1">
                  <EditBtn onClick={() => setEditingId(s.id)} />
                  <DeleteBtn onClick={() => data.removeSpac(s.id)} label="보유 스팩 삭제" />
                </span>
              </div>
            )
          })}
