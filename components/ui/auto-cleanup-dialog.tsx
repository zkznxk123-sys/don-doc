...
const TYPE_META: Record<DetectedGroupType, { label: string; icon: typeof GitMerge; color: string; bg: string; desc: string }> = {
  ...
  cancellation: { label: '결제 취소',    icon: RotateCcw,  color: 'text-amber-500',  bg: 'bg-amber-500/10 border-amber-500/20', desc: '결제 후 취소된 내역' },
                            ...
                            return (
                              <div key={tx.id} className="flex items-center justify-between gap-2">
                                <div className="min-w-0 flex items-center gap-1.5">
                                  {!willExclude && (
                                    <span className="shrink-0 text-[9px] px-1 py-0.5 rounded bg-emerald-500/10 text-emerald-500 font-medium">유지</span>
                                  )}
                                ...
                                <div className="shrink-0 text-right">
                                  <span className={cn('text-xs font-medium tabular-nums', tx.amount < 0 ? 'text-foreground/80' : 'text-emerald-500')}>
