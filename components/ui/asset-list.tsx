...
function AssetRow({
  ...
  const infoContent = (
    <>
      ...
      <div className="flex-1 min-w-0">
        {/* flex-wrap 금지 — 명의 배지가 줄바꿈되면 동명 계좌 구분 표식이 무너진다 (2026-08-13) */}
        <div className="flex items-center gap-1.5">
          <p className="text-sm font-medium text-foreground truncate">{account.name}</p>
          {account.isJoint ? (
            <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-md text-savings bg-savings-soft">공동</span>
          ) : (() => {
