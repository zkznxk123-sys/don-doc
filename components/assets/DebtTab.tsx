...
const DEBT_TYPE_META: Record<string, { label: string; color: string; bg: string }> = {
  MORTGAGE:        { label: '주담대',     color: 'text-destructive',       bg: 'bg-expense-soft' },
  JEONSE_DEPOSIT:  { label: '전세대출',   color: 'text-warning',           bg: 'bg-warning-soft' },
  CREDIT_LOAN:     { label: '신용대출',   color: 'text-destructive',       bg: 'bg-expense-soft' },
  OVERDRAFT:       { label: '마이너스통장', color: 'text-destructive',       bg: 'bg-expense-soft' },
      ...
      {summary && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <p className={cn('text-lg font-bold tabular-nums', hasEstimated && summary.totalMonthlyPayment === 0 ? 'text-warning' : 'text-foreground')}>
