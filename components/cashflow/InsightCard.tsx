...
export function InsightCard({ label, icon, actual, target, type, isRate = false }: {
  ...
  if (hasTarget) {
    if (type === 'income') {
      if (pct >= 100) { barColor = 'bg-(--viz-sage)'; valueColor = 'text-income'; statusText = '목표 달성!' }
      else if (pct >= 70) { barColor = 'bg-(--viz-copper)'; valueColor = 'text-warning'; statusText = `${pct}% 달성` }
      else { barColor = 'bg-(--viz-terra)'; valueColor = 'text-expense'; statusText = `${pct}% 달성` }
    } else if (type === 'expense') {
      if (pct <= 80) { barColor = 'bg-(--viz-sage)'; valueColor = 'text-income'; statusText = '절약 중!' }
      else if (pct <= 100) { barColor = 'bg-(--viz-copper)'; valueColor = 'text-warning'; statusText = `${pct}% 사용` }
      else { barColor = 'bg-(--viz-terra)'; valueColor = 'text-expense'; statusText = `초과 ${pct - 100}%` }
    } else {
      if (pct >= 100) { barColor = 'bg-(--viz-sage)'; valueColor = 'text-income'; statusText = '목표 달성!' }
      else if (pct >= 70) { barColor = 'bg-(--viz-copper)'; valueColor = 'text-warning'; statusText = `${pct}% 달성` }
      else { barColor = 'bg-(--viz-terra)'; valueColor = 'text-expense'; statusText = `${pct}% 달성` }
