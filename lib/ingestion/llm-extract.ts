...
## 연금·퇴직연금 계좌 (중요)
- 화면 제목·계좌명에 '연금·퇴직연금·IRP·연금저축·DC가입자·DB'가 있으면, 그 계좌의 자산은 보유 ETF·펀드까지 **전부 type="PENSION"**이다. 연금 래퍼 안의 상품은 단순 주식·펀드(INVESTMENT)가 아니다.
- **중복 금지**: 그 계좌의 대표 총액(예: "연금자산 6,077,773원" / "퇴직연금 자산 총자산 28,240,596원")이 보이면 **그 총액 1줄만** PENSION으로 넣어라. 같은 계좌 안의 개별 보유 상품(예: "RISE 단기특수은행채액티브")은 그 총액의 구성요소이므로 따로 넣지 마라 — 같은 돈을 두 번 넣으면 안 된다.
- 단, 화면에 대표 총액이 없고 개별 보유 ETF·펀드만 보이면 그것들을 각각 PENSION으로 넣어라.
