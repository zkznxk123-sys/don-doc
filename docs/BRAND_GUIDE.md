...
돈Doc은 "자산 관리 시스템"(기능)이 아니라 **돈 관리를 어렵게 만드는 모든 것에 대한 반항**(태도)이다. 나이키 "Just Do It"이 운동화가 아니라 태도를 팔듯, 돈Doc의 북극성은 기능이 아니라 **"복잡한 돈, 단순하게"**라는 태도다.

**2층 브랜드 아키텍처** — 우산(태도, 전 제품 공통) + 제품별 설명:

```
[우산·태도]  복잡한 돈, 단순하게.
   ├─ 자산 버전    "가장 쉬운 자산 관리 — 흩어진 자산을 한 화면에"
   └─ 공모주 버전  "가장 쉬운 공모주 — 청약을 한 화면에"
```

- 우산 단어는 **"돈"** — 브랜드명(돈Doc)과 일치, 자산·공모주·투자를 다 덮음. "자산"은 좁고("공모주" 어색), "투자"는 유사투자자문 컴플라이언스 리스크.
- **공모주/스팩 버전 = 돈Doc 한 우산 아래** (같은 브랜드, 진입점만 별도 커뮤니티). 자산 데이터와 연결 자연스러움.
- 부부·프라이버시·해석은 버리지 않고 이 태도 아래 **기둥**으로 유지.

### Primary Tagline (자산 버전 설명)
> **가장 쉬운 자산 관리 — 흩어진 자산을 한 화면에.**

- 북극성의 하위 = 자산 버전 **설명**. Hero, SEO description, OG image, App Store description, 채널 bio 에 사용
...
**북극성 목소리 4원칙** (2026-07-06, "복잡한 돈, 단순하게"에서 파생):
